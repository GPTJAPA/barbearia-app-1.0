const API_URL = "http://127.0.0.1:5000/servicos";
...
async function mostrarHorarios() {
  ...
  try {
    const resposta = await fetch(`http://127.0.0.1:5000/horarios-ocupados?data=${data}`);
...
async function cancelarAgendamentoCliente() {
    ...
    try {
        const resposta = await fetch(`http://127.0.0.1:5000/cancelar-telefone/${telefoneLimpo}`, {
            method: 'DELETE'
        });
